...
*
*
*	to work with rc_mt_2000_11 project
*
*******************************************/
...
#define STEPS_TO_WRITE		2			// через это значение шагов сравниваем координаты и выводим в файл
...
int main(int argc, char *argv[])
{
	...
	int error = 0;
	if (flag_compare==1){
		f_p = fopen ("MT_coords_CPU_11.txt","w");
		if (f_p==NULL) {
...
#ifndef CPU	
	f_p1 = fopen ("MT_coords_FPGA_11.txt","w");
	if (f_p1==NULL) {
	...
	*/
	...
	for(int k=0; k<N; k++) {
		...
		#ifndef CPU	
		get_time(&tt1);
		mt_fpga(dev,STEPS_TO_WRITE,1, seeds, x_2,y_2,t_2,x_2,y_2, t_2);
		get_time(&tt2);
		...
		if (flag_compare==1){
			...
			if (err) {
				error += err;
				printf("Compare results failed at step = %d, local error = %d, errors = %d\n", k, err, error);
			}
		}
#else		
		printf("Step %d\n\t CPU Time = %f\n",k,dt_c[k] );
...
{	
	...
	if (flag_rand==1) 	 {
		...
		// set rand and seed enable params in hls core
		RD_WriteDeviceReg32m(dev, CNTRL_BAR, HLS_B, 1);			// rand enable
		RD_WriteDeviceReg32m(dev, CNTRL_BAR, HLS_LENGTH, flag_seed);	// seed enable
		
	} else {
		
		RD_WriteDeviceReg32m(dev, CNTRL_BAR, HLS_B, 0);
		RD_WriteDeviceReg32m(dev, CNTRL_BAR, HLS_LENGTH, 0);
...
void print_usage(char *argv[])
{
	printf("Usage:\n");
	printf("\n--------------------------------------------------------------------------\n");
	printf("To run HLS TEST:\nsudo %s -b [board] -v [chip_select] -N [N_d] [-compare/ ] [-rand/ ]\n", argv[0]);	
